...
export default function ConfirmDeleteCustomerModal({ customerId, company }) {
  const { deleteCustomer, isAdmin } = useAppState();
  ...
  async function handleDelete() {
    try {
      ...
      navigate(isAdmin ? '/customers' : '/my-customers');
    } catch (e) {
      toast(e.message);
    }
  }
    ...
    <div className="elg-modal deleteEmployeeModal" style={{ maxWidth: 560 }}>
      ...
      <div className="elg-modal-body">
        <p style={{ color: '#5C5C5C', margin: 0, lineHeight: 1.5 }}>
          You're deleting the customer <b style={{ fontWeight: 500, color: 'var(--elg-ink)' }}>"{company || 'this customer'}."</b> This action can't be undone. If they still have orders or invoices on record, you'll need to remove those first.
        </p>
      </div>
